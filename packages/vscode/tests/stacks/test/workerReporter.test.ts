...
import { ProgressReporter } from '../../../src/stacks/test/worker/reporter';

let resultAcknowledgement = Promise.withResolvers<void>();

rs.mock('../../../src/stacks/test/worker', () => {
  const acknowledged = () => resultAcknowledgement.promise;
  const event = Object.assign(acknowledged, { asEvent: async () => {} });
  const immediate = Object.assign(async () => {}, { asEvent: async () => {} });
  return {
    masterApi: {
