// Ported from web-infra-dev/rslint `packages/vscode-extension/src/RuntimeManager.ts`
// (rslint #1617, commit 39536fd6). The lifecycle logic — refcounting a runtime
// by open document, start-before-switch handoff, forward-only cleanup, the
// per-document serialized tail and epochs — is upstream's and must stay
// diffable against it.
//
...
//   The last-good rule is upstream's — the document keeps the runtime it
//   already had, and a never-bound document is simply not linted.
...
export interface DocumentResolutionFailure {
...
export class RuntimeManager {
  ...
  private async reconcileCurrentDocument(
    ...
    if (existing) {
      void this.releaseRuntime(existing, key).catch((error: unknown) => {
        this.logger.error(
          `Failed to close superseded Rslint core ${existing.resolved.installation.packageDirectory}`,
          error,
        );
      });
    }
  ...
  private async startRuntime(
    ...
    entry.active = true;
    this.logger.info(
      `Rslint core ${entry.resolved.installation.version ?? 'unknown'} ready for ${entry.resolved.workspaceFolder.name}`,
    );
  }
    ...
    if (!entry.closePromise) {
      ...
      const closePromise = (async () => {
        ...
        try {
          ...
          this.options.onRuntimeClosed?.(entry.resolved);
        }
  ...
  private async releaseRuntimeAfterFailure(
    ...
    try {
      ...
      this.logger.error(
        `Failed to clean up unusable Rslint core ${entry.resolved.installation.packageDirectory}`,
        error,
      );
    }
  }
  ...
  /**
   * Upstream shows one deduplicated warning toast per workspace and failure
