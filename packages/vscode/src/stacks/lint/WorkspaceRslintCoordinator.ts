...
export class WorkspaceRslintCoordinator {
  ...
  public constructor(
    ...
    /**
     * Classifies a start rejection as an expected outcome instead of a failure.
     * A root that deliberately does not start (for example a folder with no
     * lint configuration at all) still rejects — that is how a slot is kept
     * from being retried — but it must not be logged as an error.
     */
    private readonly isExpectedStartFailure: (error: unknown) => boolean = () =>
      false,
  ) {}
  ...
  private async startDesired(
    ...
    try {
      ...
      if (!stale) {
        slot.failedGeneration = desired.generation;
        desired.readiness.reject(error);
        if (this.isExpectedStartFailure(error)) {
          this.logger.info(
            `Rslint workspace ${slot.key} was not started: ${describeError(error)}`,
          );
        } else {
          this.logger.error(
