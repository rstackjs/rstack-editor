...
export class RstestApi {
  ...
  constructor(
     ...
     * rstack shim (`node_modules/rstack/dist/rstestConfig.js`) can be the
     * config file while the worker still runs in the `rstack.config.*`
     * directory. For a native `rstest.config.*` the caller passes
     * `dirname(configFilePath)`, which is byte-identical to upstream.
     */
    private cwd: string,
  ...
  private resolveRstestPath(): string {
    try {
      ...
      // extension is versioned independently and the support matrix is the
      // whole compatibility contract, so the comparison is dropped.
      //
