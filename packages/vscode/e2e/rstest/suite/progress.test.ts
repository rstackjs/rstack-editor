// Ported from upstream `rstest/packages/vscode/tests/suite/progress.test.ts`.
//
// Adaptations: exports come through the shell's exports channel
// (`whenStackActive('rstest')`), and the fixture path is resolved through
// `FIXTURES_ROOT` (the compiled suite lives deeper under `tests-dist/` than
// upstream's). All progress/diagnostic assertions are upstream's, unchanged —
...
suite('Test Progress Reporting', () => {
  ...
  test('can run a single test case', async () => {
    ...
    const progressFileUri = vscode.Uri.file(
      path.resolve(FIXTURES_ROOT, 'workspace-1/test/progress.test.ts'),
    );
    assert.equal(
