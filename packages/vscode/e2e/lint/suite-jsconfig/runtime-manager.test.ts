...
// `packages/vscode-extension/__tests__/suite-jsconfig/runtime-manager.test.ts`
// (rslint #1617, commit 39536fd6). It replaces this suite's former
// `workspace-coordinator.test.ts`: the coordinator it exercised no longer
// exists, and the lifecycle it covered is now the RuntimeManager's.
//
...
//   loads `@rslint/core` — ADR 0003), so the fake installation has no
//   `protocolVersion`/binary and adds `mode`.
// - Assertion semantics are upstream's, unchanged.
...
import type {
  CoreInstallation,
  ResolvedCoreRuntime,
} from '../../../src/stacks/lint/CoreResolver';
import {
  RuntimeManager,
...
import { WorkspaceDocumentRouter } from '../../../src/stacks/lint/WorkspaceDocumentRouter';
...
class FakeResolver implements RuntimeCoreResolver {
  ...
  async resolve(
    ...
    // The fake runtime never spawns a worker, so no real core directory is
    // needed — only a stable physical identity.
    ...
    return {
      key: `${workspaceFolder.uri.toString()}\0${identity}`,
      workspaceFolder,
      installation,
    };
  }
...
suite('local-core runtime manager', () => {
  ...
  setup(() => {
    ...
    // Deviation: upstream also parks the folder in a suite-level
    // `workspaceFolder` binding it never reads. This repo's
    // `@typescript-eslint/no-unused-vars` rejects that, so only the local
    // `folder` survives; the fake resolver receives the folder the manager
    // passes it, exactly as upstream.
