...
// `packages/vscode-extension/__tests__/suite-jsconfig/jsconfig.test.ts`
// (origin/main). Adaptations:
// - The fixture still ships `rslint.json` so the JS-over-JSON priority test
//   keeps its upstream server-side meaning, but `rslint.json` is NOT a
//   detection signal in this extension.
// - "deleting JS config should clear diagnostics" is adapted: deleting the
//   last `rslint.config.*` now un-detects the folder, the shell deregisters
//   the whole lint stack and every rslint diagnostic is dropped — the server
//   never gets a chance to fall back to `rslint.json` (JSON configs are
//   deprecated upstream and unsupported here).
...
`;
    await withFailClosedCleanup(
      async () => {
        ...
        writeFileAtomicSync(configPath, newConfig, 'utf8');
      ...
      async () => {
        ...
        writeFileAtomicSync(configPath, originalConfig, 'utf8');
        await restored;
      },
...
`;
    ...
    await withFailClosedCleanup(
      async () => {
        ...
        writeFileAtomicSync(configPath, countedConfig, 'utf8');
        await reloaded;
  ...
  test('deleting JS config should clear diagnostics', async () => {
    ...
    await withFailClosedCleanup(
      async () => {
        // Adapted expectation: the deleted file was the only
        // detection signal, so the shell must deregister the lint stack —
        // `rslint.json` on disk is not a config and cannot keep it alive.
        fs.unlinkSync(configPath);
        await waitForLintStackRegistration(false);
        // Deregistration disposes the language client, which drops its
        // diagnostics collection: the JSON fallback rule must not survive.
        const afterDeleteDiags = await waitForDiagnostics(
          doc,
          (diags) => diags.length === 0,
        );
        assert.strictEqual(
          afterDeleteDiags.length,
          0,
          'After deleting the last JS config, all rslint diagnostics must be gone',
        );
      },
      async () => {
...
`;
    ...
    await withFailClosedCleanup(
      async () => {
        ...
        writeFileAtomicSync(nestedFilePath, 'debugger;\n', 'utf8');
        writeFileAtomicSync(rootConfigPath, rootConfigWithMarker, 'utf8');
...
`,
        ...
        writeFileAtomicSync(postFailureFilePath, 'debugger;\n', 'utf8');
        postFailureDoc =
  ...
  test('parent global ignores remove nested configs from the effective catalog', async () => {
    ...
    await withFailClosedCleanup(
      async () => {
        fs.mkdirSync(nestedDir, { recursive: true });
        writeFileAtomicSync(nestedFilePath, 'console.log("nested");\n', 'utf8');
        writeFileAtomicSync(
          nestedConfigPath,
          `import fs from 'node:fs';
...
`,
        ...
        writeFileAtomicSync(rootConfigPath, ignoredRootConfig, 'utf8');
  ...
  test('config search excludes node_modules and .git', async () => {
    ...
    await withFailClosedCleanup(
      async () => {
        ...
        writeFileAtomicSync(rootConfigPath, changedRootConfig, 'utf8');
        await reloaded;
...
`;
    ...
    await withFailClosedCleanup(
      ...
      async () => {
        ...
        writeFileAtomicSync(jsPath, originalJS, 'utf8');
...
`;
    ...
    await withFailClosedCleanup(
      async () => {
        writeFileAtomicSync(mjsPath, lowerPriorityConfig, 'utf8');
        writeFileAtomicSync(
          jsPath,
          `import fs from 'node:fs';
