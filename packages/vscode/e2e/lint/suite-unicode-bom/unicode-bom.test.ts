...
// that field below preserves the upstream no-var positive control and
// unicode-bom absence assertion after the message prefix is stripped.
...
suite('rslint unicode-bom over LSP', function () {
  ...
  async function lintedDiagnostics(
    ...
    return waitForRslintDiagnostics(doc, (all) =>
      all.some((d) => diagnosticRuleIdIncludes(d, 'no-var')),
    );
  }

  suiteSetup(async () => {
    await writeFileAtomic(fixturePath('marked.ts'), BOM + markedSource, 'utf8');
  });
  ...
  test('reports nothing about the mark on a marked file', async () => {
    ...
    assert.deepStrictEqual(
      diagnostics
        .filter((d) => diagnosticRuleIdIncludes(d, 'unicode-bom'))
        .map((d) => d.message),
      [],
