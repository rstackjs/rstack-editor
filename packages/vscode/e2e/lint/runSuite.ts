/**
 * The in-Extension-Host runner shared by every lint suite (upstream
 * `rslint/packages/vscode-extension/__tests__/runSuite.ts`).
 *
 * Adaptations from upstream:
 * - Extension ID is `rstack.rstack` (the unified shell).
 * - Upstream's `extension.activate()` resolved only once a language-server
 *   root was ready. Our shell activates without blocking on a server start
 *   (the shell-activation adaptation), so suites that expect a lint runtime wait
...
export interface RunSuiteOptions {
  /**
   * Whether the fixture workspace is expected to light the Rslint stack.
   * Defaults to true: the harness then blocks until the shell registered the
   * lint controller, which mirrors upstream's activation contract. The
   * no-config suite passes false — its whole point is that the stack never
   * registers (rslint is not zero-config).
   */
  readonly expectLintStack?: boolean;
}

type RunCallback = (error: unknown, failures?: number) => void;

export function createRun(
  options: RunSuiteOptions = {},
): (testPath: string, callback: RunCallback) => void {
  return (testPath, callback) => {
    void activateAndRun(testPath, options, callback);
  };
}
...
async function activateAndRun(
  ...
  try {
    ...
    // Extension.activate() is the shell's readiness contract: detection ran
    // and the initial reconcile pass registered every gated stack.
    const extensionExports = await runBeforeDeadline(
      () => extension.activate() as Promise<RstackExtensionExports>,
      startupDeadline,
      `activation of ${EXTENSION_ID}`,
    );
    if (options.expectLintStack !== false) {
      // The upstream readiness contract, restated on the shell architecture:
      // do not load a single test while the lint controller is unregistered.
      await runBeforeDeadline(
        () => extensionExports.whenStackActive('rslint'),
        startupDeadline,
        'the Rslint stack to register (is the fixture config detected?)',
      );
    }
    await activateBuiltInCodeActionExtensions(startupDeadline);
    await waitForTypeScriptCodeActionProviders(startupDeadline);

    const files = collectTests(testPath);
    if (files.length === 0) {
